...
try:
    from utils_export import export_schedule_pdf, export_schedule_xlsx
except Exception:
...
def export_xlsx(payload: LeaseInput):
    res = _load_build_schedule()(payload)
    data = export_schedule_xlsx(
        res.get("schedule", []), currency=getattr(payload, "currency", "USD")
    )
    return Response(
        content=data,
        media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
...
@app.get("/__probe")
def __probe():
    return {"debug_errors": os.getenv("DEBUG_ERRORS", "0"), "has_handler": True}
